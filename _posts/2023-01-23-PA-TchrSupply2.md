...
Teacher supply is a major issue facing the education system. This report
focuses on teacher supply components in Pennsylvania.
...
Manual</a> in the “Staff Template” section. Addition documentation on
the staff data are at PDE’s
<a href="https://www.education.pa.gov/DataAndReporting/ProfSupPers/Pages/ProfPersIndStaff.aspx">data
page.</a>
...
Other important measures in these data are `PublicID` (assigned by PDE
as the unique identifier for each record), `Position` (names the
education-related occupation for each record; most are teachers), and
`AUN` (the standard identifier for a school district).
...
number of teachers in each year of the dataset by their mobility status
(Table 1). Take note that the number of “NA” records in 2020-21 is quite
large because computation of mobility status depends on 2021-22 data,
...
Table 1: Number of Teachers by Mobility Status and Year
</td>
</tr>
<tr>
...
that set that also appear in a subsequent year.
...
``` r
# render("2022-10-05-PA-TchrSupply.Rmd",md_document(variant="markdown_github"))
```

[^1]: Often these ‘other’ assignments are for langauge, arts, technology
